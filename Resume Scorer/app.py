...
class ResumeScorerHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/score':
            ...
            try:
                ...
                if resume_file:
                    resume_text = self.parse_resume_file(resume_file, file_type)
                
                # Validate inputs
                if not job_text or not resume_text:
                    self.send_json_response({'error': 'Missing job description or resume'}, 400)
                    return
    ...
    def parse_resume_file(self, file_data, file_type):
...
def main():
    server_address = ('', 8080)
    httpd = HTTPServer(server_address, ResumeScorerHandler)
    
    print("\n" + "="*60)
    print("Resume Scorer App - Running!")
    print("="*60)
